...
menu_child = 0 
menu_child_switch = 0
# http://www.dakstats.com/WebSync/Pages/Team/IndividualStats.aspx?association=10&sg=WVB&sea=NAIWVB_2019&team=2030
...
class UI(Frame):
    ...
    def constructButtons(self, choice):
        ...
        if (choice == 5):
            self.convertButton = Button(win, text="Convert Women Volleyball Stats", command=self.convertWomanVolleyball)
        if (choice == 6):
    ...
    def convertWomanVolleyball(self):
        # Reading docx file
        import sys
        from docx import Document
        from docx.shared import Pt
        ...
        url = "http://www.dakstats.com/WebSync/Pages/Team/IndividualStats.aspx?association=10&sg=WVB&sea=NAIWVB_2019&team=2030"
        page = urllib2.urlopen(url)
        soup = BeautifulSoup(page, "html.parser")
        table = soup.find("table", {"class": "gridViewReportBuilderWide"})
        names = []
                    ...
                    for row in working.rows:
                        for cell in row.cells:
                            ...
                            if flag > 3:
                                ...
                                if paragraph.text.startswith('MP-'):
                                    print 'MP found'
                                ...
                                if paragraph.text.startswith('SA-'):
                                    print 'SA found'
                                    paragraph.text = "SA- " + SA
                                    print paragraph.text

                                if paragraph.text.startswith('SE-'):
                                    print 'SE found'
                                    paragraph.text = "SE- " + SE
                                    print paragraph.text

                                if paragraph.text.startswith('AST-'):
                                    print 'A found' + A
        ...
        print " Information might be wrong due to multiple first names appearing."
        document.add_table(1, 2, style=None)
        document.save(newDocDirectory + "/Womens Volleyball Updated File.docx")
        sys.exit()
